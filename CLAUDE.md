...
- **Scenes** are deliberately thin: each `.tscn` is just a root node + script;
  ...
  - `scenes/rhythm/rhythm_game.gd` — spawn/move/judge notes, pay Beats.
    Plays `SongData`'s layered 125 BPM song **endlessly** through PDJE,
    re-issuing the chart every lap; Esc/Finish banks the run. Falls back to the
    generated chart JSON (silently) if PDJE can't start.
    `-- --selftest <seconds>` logs the loop bookkeeping and quits.
  ...
  - `scenes/tools/pdje_audio_check.gd` — dev tool: PDJE playback spike
    (InitPlayer/LoadMusic/SetMusic/ChangeBpm/CueMusic + clock accuracy).
- **Data**: `data/skills/skill_data.gd` (`SkillData`), plus two global classes:
  ...
  - `AudioBake` (`data/audio/audio_bake.gd`) — places one-shots on a grid to
    bake a loop (the four-on-the-floor kick). Tempo fitting is NOT done here;
    PDJE's `ChangeBpm()` stretches layers at runtime.
  - `SongData` (`data/songs/song_data.gd`) — the song: 125 BPM, 8 bars, the
    kick pulse plus skill-gated layers, and the chart (which gains hold notes
    once the `hold_notes` skill is owned). `PdjeSong` (`data/songs/pdje_song.gd`)
    plays the layers through PDJE's MusPanel, supplies Conductor's clock, and
    exposes `set_layer_filter()` for the realtime lowpass.
...
- CLI (`Godot_v4.7.1-stable_win64_console.exe --path .`):
  ...
  - `res://scenes/rhythm/rhythm_game.tscn -- --selftest 50` — endless-loop check.
    Add `--autoplay` to have it hit every note perfectly (holds included) and
    report combo / holds completed / holds broken.
  - `res://scenes/tools/pdje_audio_check.tscn -- --auto` — PDJE playback + FX
    spike. `--buffer N` sweeps the InitPlayer frame buffer.
