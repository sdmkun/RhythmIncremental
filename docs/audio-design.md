...
> **【重要】cue のタイミングは「生の」カウンタで測ること。**
> cue が効くのは**エンジンがいま生成している音**で、生成はスピーカーより
> 0.111 秒先行している。したがって:
> - **生カウンタ**がループ長に達した時に送る → 生成が途切れずに次の周へ続く。**正しい**
> - **補正後の位置**（＝聞こえている位置）で送る → まるまる0.111秒遅れ、隙間ができる。
>
> `PdjeSong` は `position()`（補正後・譜面用）と `_raw_position()`（生・cue用）を
> 分けている。
