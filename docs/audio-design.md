...
- **`MusPanel` は自前でループする**（非文書だが実機で確認）。
  したがって **`MANUAL_LOOP = false`。自分で `CueMusic` してはいけない** —
  上記1フレーム先行のせいで毎周ループ末尾を削ってしまう。
  `CueMusic` を使うのはクロックのゼロ点を確定する初回だけ。

補正後のループ境界（実測）: 周期はきっちり **15.360 秒**。
